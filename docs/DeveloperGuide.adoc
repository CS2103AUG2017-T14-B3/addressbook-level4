...
SortCommandParser that processes the user input command that performs the suitable sorting method to achieve the user
wants. Since this enhancement is not able to reuse some of the method in other components, new methods have to be
created and implemented.

The user input command will be passed to the SortCommandParser which parses the input by checking to see if it is
 ...
 *executeUndoableCommand* by calling the sortPerson method in the *ModelManager* class which will call the sortPerson
 ...
 parameter passed in in each method are the input string which indicates the sortType of sorting the user wants/needs.

In *UniquePersonList*, the string passed will then be checked using a `switch case`. If the sortType is found, it will
perform the corresponding sorting algorithm. If not it will throw a *InvalidSortTypeException* which tells the user
that the sorting sortType the user keyed in does not exist.
