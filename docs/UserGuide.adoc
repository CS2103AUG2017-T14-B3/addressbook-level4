...
[NOTE]
====
1. Sorting by *priority* sorts the list by *_H, M and L_* respectively. After sorting by H, M and L, they will then
sort them alphabetically by their names.

2. Sorting by *position* sorts the list by making all the *_NIL_* position to be at the bottom of the list. After doing
