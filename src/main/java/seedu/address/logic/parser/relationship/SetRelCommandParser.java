...
import static seedu.address.logic.parser.CliSyntax.PREFIX_REMOVE_RELATIONSHIP;
...
public class SetRelCommandParser implements Parser<SetRelCommand> {
    ...
    public SetRelCommand parse(String args) throws ParseException {
        ...
        ArgumentMultimap argMultimap = ArgumentTokenizer.tokenize(args, PREFIX_ADD_RELATIONSHIP,
            PREFIX_CLEAR_RELATIONSHIP, PREFIX_REMOVE_RELATIONSHIP);
        Index indexOne;
        Index indexTwo;
        String value;
        String[] indexes;
        ...
        try {
            ...
            parseRelForEdit(argMultimap.getAllValues(PREFIX_REMOVE_RELATIONSHIP)).ifPresent
                (editPersonDescriptor::setToRemove);
            editPersonDescriptor.setClearRels(argMultimap.containsPrefix(PREFIX_CLEAR_RELATIONSHIP));
        } catch (IllegalValueException ive) {
