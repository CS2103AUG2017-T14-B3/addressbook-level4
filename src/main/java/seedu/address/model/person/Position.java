...
public class Position implements Comparator<Position> {
     ...
     */
    public int compareTo(Position positionOne) {
        if (ORDERED_ENTRIES.contains(positionOne.toString()) && ORDERED_ENTRIES.contains(this.toString())) {
            return ORDERED_ENTRIES.indexOf(positionOne.toString()) - ORDERED_ENTRIES.indexOf(this.toString());
        }
        if (ORDERED_ENTRIES.contains(positionOne.toString())) {
            return 1;
        }
        if (ORDERED_ENTRIES.contains(this.toString())) {
            return -1;
        }
        return positionOne.toString().compareTo(this.toString());
    }
    @Override
